...
public class PowerUp : MonoBehaviour
{
    ...
    [Header("Dynamic")]
    public eWeaponType _type;  // maybe
    ...
    public void SetType(eWeaponType wt){    //error here some sort of infinite loop
        WeaponDefinition def = Main.GET_WEAPON_DEFINITION(wt);
        //Debug.LogError("def is" + def.powerUpColor);
        cubeMat.color = def.powerUpColor;
        //Debug.LogError("def is" + def.letter);

        letter.text = def.letter;
        //Debug.LogError("def is" + letter.text);
        type = wt;
    }
